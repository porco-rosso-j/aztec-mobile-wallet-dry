...
// returns the same io_funcType or replaces it with an equivalent that's already
// in the type linked list
void Environment_AddFuncType(IM3Environment i_environment,
                             IM3FuncType *io_funcType) {
...
IM3CodePage RemoveCodePageOfCapacity(M3CodePage **io_list,
                                     u32 i_minimumLineCount) {
...
IM3CodePage Environment_AcquireCodePage(IM3Environment i_environment,
                                        u32 i_minimumLineCount) {
  return RemoveCodePageOfCapacity(&i_environment->pagesReleased,
                                  i_minimumLineCount);
}

void Environment_ReleaseCodePages(IM3Environment i_environment,
                                  IM3CodePage i_codePageList) {
...
IM3Runtime m3_NewRuntime(IM3Environment i_environment, u32 i_stackSizeInBytes,
  ...
  if (runtime) {
    ...
    runtime->stack =
        m3_Malloc("Wasm Stack",
                  i_stackSizeInBytes +
                      4 * sizeof(m3slot_t)); // TODO: more precise stack checks
...
void *ForEachModule(IM3Runtime i_runtime, ModuleVisitor i_visitor,
                    void *i_info) {
  void *r = NULL;
...
M3Result EvaluateExpression(IM3Module i_module, void *o_expressed, u8 i_type,
  ...
  //  OPTZ: this code page could be erased after use.  maybe have 'empty' list
  //  in addition to full and open?
...
M3Result InitGlobals(IM3Module io_module) {
  ...
  if (io_module->numGlobals) {
    // placing the globals in their structs isn't good for cache locality, but i
    // don't really know what the global access patterns typically look like
    // yet.

    //          io_module->globalMemory = m3Alloc (m3reg_t,
    ...
    //          if (io_module->globalMemory)
    {
      for (u32 i = 0; i < io_module->numGlobals; ++i) {
        ...
        if (g->initExpr) {
          ...
          result = EvaluateExpression(io_module, &g->i64Value, g->type, &start,
                                      g->initExpr + g->initExprSize);
    ...
    //          else result = ErrorModule (m3Err_mallocFailed, io_module, "could
    //          allocate globals for module: '%s", io_module->name);
  }
...
//
// install in react-native-webassembly.cpp
// runtime::load
// load_into
// load_into in wasm3_cpp.h
//  m3_LoadModule
//  InitDataSegments
...
M3Result InitDataSegments(M3Memory *io_memory, IM3Module io_module) {
  ...
  for (u32 i = 0; i < io_module->numDataSegments; ++i) {
    ...
    m3log(runtime, "loading data segment: %d; size: %d; offset: %d", i,
          segment->size, segmentOffset);

    if (segmentOffset >= 0 && (size_t)(segmentOffset) + segment->size <=
                                  io_memory->mallocated->length) {
  ...
  for (u32 i = 0; i < io_module->numElementSegments; ++i) {
    ...
    if (index == 0) {
      ...
      if (endElement > io_module->table0Size) {
        io_module->table0 = m3_ReallocArray(IM3Function, io_module->table0,
                                            endElement, io_module->table0Size);
...
M3Result m3_RunStart(IM3Module io_module) {
  ...
  if (io_module and io_module->startFunction >= 0) {
    ...
    result = (M3Result)RunCode(function->compiled, (m3stack_t)runtime->stack,
                               runtime->memory.mallocated, d_m3OpDefaultArgs);
...
M3Result m3_LoadModule(IM3Runtime io_runtime, IM3Module io_module) {
  ...
  // so it will be called before a function call is attempted (in
  // m3_FindFunction)
...
M3Result m3_FindFunction(IM3Function *o_function, IM3Runtime i_runtime,
  ...
  if (function) {
    ...
    _throw(ErrorModule(m3Err_functionLookupFailed, i_runtime->modules, "'%s'",
                       i_functionName));
...
M3Result m3_CallVL(IM3Function i_function, va_list i_args) {
  ...
  result = (M3Result)RunCode(i_function->compiled, (m3stack_t)(runtime->stack),
                             runtime->memory.mallocated, d_m3OpDefaultArgs);
...
void ReleaseCodePageNoTrack(IM3Runtime i_runtime, IM3CodePage i_codePage) {
  if (i_codePage) {
    ...
    m3log(emit, "release page: %d to queue: '%s'", i_codePage->info.sequence,
          pageFull ? "full" : "open")
  }
}

IM3CodePage AcquireCodePageWithCapacity(IM3Runtime i_runtime,
                                        u32 i_minLineCount) {
  IM3CodePage page =
...
void ReleaseCodePage(IM3Runtime i_runtime, IM3CodePage i_codePage) {
  if (i_codePage) {
    ...
    m3log(runtime,
          "runtime: %p; open-pages: %d; full-pages: %d; active: %d; total: %d",
          i_runtime, numOpen, numFull, i_runtime->numActiveCodePages,
          i_runtime->numCodePages);

    d_m3Assert(numOpen + numFull + i_runtime->numActiveCodePages ==
               i_runtime->numCodePages);
...
M3Result m3Error(M3Result i_result, IM3Runtime i_runtime, IM3Module i_module,
  ...
  if (i_runtime) {
    ...
    vsnprintf(i_runtime->error_message, sizeof(i_runtime->error_message),
              i_errorMessage, args);
    va_end(args);
  }
