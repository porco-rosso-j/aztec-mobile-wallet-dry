...
M3Result ParseType_Memory(M3MemoryInfo *o_memory, bytes_t *io_bytes,
                          cbytes_t i_end) {
  M3Result result = m3Err_none;
...
M3Result ParseSection_Function(IM3Module io_module, bytes_t i_bytes,
                               cbytes_t i_end) {
  M3Result result = m3Err_none;
...
M3Result ParseSection_Import(IM3Module io_module, bytes_t i_bytes,
                             cbytes_t i_end) {
  M3Result result = m3Err_none;
  ...
  for (u32 i = 0; i < numImports; ++i) {
    ...
    m3log(parse, "    kind: %d '%s.%s' ", (u32)importKind, import.moduleUtf8,
          import.fieldUtf8);
    switch (importKind) {
    ...
    case d_externalKind_global: {
      ...
      m3log(parse, "     global: %s mutable=%d", c_waTypes[type],
            (u32)isMutable);

      IM3Global global;
      _(Module_AddGlobal(io_module, &global, type, isMutable,
                         true /* isImport */));
...
M3Result ParseSection_Export(IM3Module io_module, bytes_t i_bytes,
                             cbytes_t i_end) {
  M3Result result = m3Err_none;
  ...
  for (u32 i = 0; i < numExports; ++i) {
    ...
    m3log(parse, "    index: %3d; kind: %d; export: '%s'; ", index,
          (u32)exportKind, utf8);
...
M3Result ParseSection_Start(IM3Module io_module, bytes_t i_bytes,
                            cbytes_t i_end) {
  M3Result result = m3Err_none;
...
M3Result Parse_InitExpr(M3Module *io_module, bytes_t *io_bytes,
                        cbytes_t i_end) {
  M3Result result = m3Err_none;

  // this doesn't generate code pages. just walks the wasm bytecode to find the
  // end
...
#endif
  compilation = (M3Compilation){.runtime = NULL,
                                .module = io_module,
                                .wasm = *io_bytes,
                                .wasmEnd = i_end};
...
M3Result ParseSection_Element(IM3Module io_module, bytes_t i_bytes,
                              cbytes_t i_end) {
  M3Result result = m3Err_none;
  ...
  _throwif("too many element segments",
           numSegments > d_m3MaxSaneElementSegments);
...
M3Result ParseSection_Code(M3Module *io_module, bytes_t i_bytes,
                           cbytes_t i_end) {
  M3Result result;
...
M3Result ParseSection_Data(M3Module *io_module, bytes_t i_bytes,
                           cbytes_t i_end) {
  M3Result result = m3Err_none;
  ...
  for (u32 i = 0; i < numDataSegments; ++i) {
    ...
    m3log(parse, "    segment [%u]  memory: %u;  expr-size: %d;  size: %d", i,
          segment->memoryRegion, segment->initExprSize, segment->size);
...
M3Result ParseSection_Memory(M3Module *io_module, bytes_t i_bytes,
                             cbytes_t i_end) {
  M3Result result = m3Err_none;
...
M3Result ParseSection_Global(M3Module *io_module, bytes_t i_bytes,
                             cbytes_t i_end) {
  M3Result result = m3Err_none;
  ...
  for (u32 i = 0; i < numGlobals; ++i) {
    ...
    _(NormalizeType(&type, waType));
    _(ReadLEB_u7(&isMutable, &i_bytes, i_end));
    m3log(parse, "    global: [%d] %s mutable: %d", i, c_waTypes[type],
          (u32)isMutable);
...
M3Result ParseSection_Name(M3Module *io_module, bytes_t i_bytes,
                           cbytes_t i_end) {
  M3Result result;
    ...
    if (nameType == 1) {
      ...
      for (u32 i = 0; i < numNames; ++i) {
        ...
        if (index < io_module->numFunctions) {
          ...
          //                          else m3log (parse, "prenamed: %s",
          //                          io_module->functions [index].name);
        }
...
M3Result ParseSection_Custom(M3Module *io_module, bytes_t i_bytes,
                             cbytes_t i_end) {
  M3Result result;
  ...
  if (strcmp(name, "name") == 0) {
    ...
    _(io_module->environment->customSectionHandler(io_module, name, i_bytes,
                                                   i_end));
  }
