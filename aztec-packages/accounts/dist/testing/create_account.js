 ...
 */
function createAccount(pxe) {
  const secretKey = _circuits.Fr.random();
  const signingKey = (0, _circuits.deriveSigningKey)(secretKey);
  return (0, _index.getSchnorrAccount)(pxe, secretKey, signingKey).waitSetup();
}
/**
 ...
 */
async function createAccounts(pxe, numberOfAccounts = 1, secrets = []) {
  const accounts = [];
  if (secrets.length == 0) {
    secrets = Array.from({
  ...
  for (const secret of secrets) {
    const signingKey = (0, _circuits.deriveSigningKey)(secret);
    const account = (0, _index.getSchnorrAccount)(pxe, secret, signingKey);
