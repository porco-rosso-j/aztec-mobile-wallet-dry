...
export function concatenateUint8Arrays(arrayOfUint8Arrays: Uint8Array[]) {
  const totalLength = arrayOfUint8Arrays.reduce(
...
export function deserializeUInt32(buf: Uint8Array, offset = 0) {
  const adv = 4;
  const elem = new DataView(buf.buffer, buf.byteOffset + offset, adv).getUint32(
    0,
    false
  );
  return { elem, adv };
...
export function deserializeInt32(buf: Uint8Array, offset = 0) {
  const adv = 4;
  const elem = new DataView(buf.buffer, buf.byteOffset + offset, adv).getInt32(
    0,
    false
  );
  return { elem, adv };
